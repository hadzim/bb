...
namespace BB {
	...
	namespace Sensor {
		...
		const std::string Info::UnitTemperature = "UnitTemperature";

		Info::Info(std::string uid, std::string type, const Settings & settings) :
				uid(uid), type(type), settings(settings){

		}
		Info::Info(std::string uid, std::string type, std::string unit, const Settings & settings):
				uid(uid), type(type), unit(unit), settings(settings){
